...
async def test(token: Token, session: Session = Depends(get_session)):
    ...
    response = requests.get("http://172.28.1.11:8080/realms/master/protocol/openid-connect/userinfo",
    ...
    user = AppUser(username=username, first_name=name, last_name=last_name, email=email, created_at=datetime.now(), updated_at=datetime.now(), is_active=True)
